...
export const createVariantOption = asyncHandler(async (req, res) => {
  ...
  await prisma.$transaction((tx) =>
    writeAudit(tx, {
      ...
      entityId: option.id,
      action: "CREATED",
      afterJson: { name: option.name, type: option.type },
    }),
  );
...
export const listInventory = asyncHandler(async (_req, res) => {
  ...
  res.json({
    variants: variants.map((variant) => ({
      ...
      option: variant.productOption.variantOption.name,
      product: variant.productOption.product.name,
      category: variant.productOption.product.category.name,
  ...
  const level = await prisma.$transaction(async (tx) => {
    ...
    if (onHand < current.reserved)
      throw new AppError(
        409,
        "RESERVED_STOCK",
        "O ajuste deixaria menos estoque físico do que peças já reservadas.",
      );
    ...
    await writeAudit(tx, {
      ...
      entityId: variantId,
      action: "ADJUSTED",
      afterJson: { delta, reason, onHand, reserved: updated.reserved },
    });
    return updated;
  });
  res.json({
    inventory: { ...level, available: level.onHand - level.reserved },
  });
});
...
export const createStaff = asyncHandler(async (req, res) => {
  ...
  const user = await prisma.user.create({
    data: { name: req.body.name, email, passwordHash, role: "STAFF" },
  });
  await prisma.$transaction((tx) =>
    writeAudit(tx, {
      actorId: req.user.id,
...
export const signedUpload = asyncHandler(async (_req, res) => {
  if (
    ...
    throw new AppError(
      503,
      "UPLOAD_UNAVAILABLE",
      "Cloudinary ainda não foi configurado.",
    );
  const timestamp = Math.floor(Date.now() / 1000);
  const folder = "ferchu-modas";
...
export const createBanner = asyncHandler(async (req, res) => {
  const banner = await prisma.$transaction(async (tx) => {
    ...
    await writeAudit(tx, {
      ...
      entityId: created.id,
      action: "CREATED",
      afterJson: { title: created.title, assetId: created.assetId },
    });
    return created;
  });
...
export const updateBanner = asyncHandler(async (req, res) => {
  const banner = await prisma.$transaction(async (tx) => {
    const before = await tx.siteBanner.findUnique({
      where: { id: req.params.id },
    });
